...
The field has also evolved from earlier white-box token-sampling schemes toward a broader design space that includes hybrid and black-box methods. The reviewed studies now span generative schemes that write stego text from scratch \cite{yang2020vae,DBLP:journals/corr/abs-2106-02011,ding2023discop,kaptchuk2021meteor}, rewriting systems that paraphrase existing covers \cite{li2023rewriting}, black-box pipelines that treat the model as an opaque API \cite{wu2024generative,steinebach2024natural}, zero-shot protocols driven primarily by prompting \cite{lin2024zero}, context-aware frameworks that retrieve external information to increase semantic plausibility or payload \cite{liao2024co,wang2023hi}, and methods that emphasize stronger security guarantees \cite{kaptchuk2021meteor,ding2023discop}.
...
\begin{table}[h]
  \centering
  \small
  \caption{Representative model families across the 26 studies explicitly enumerated in the current synthesis tables. Categories are non-exclusive, so totals exceed 100\%.}
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{4cm} >{\raggedright\arraybackslash}p{9cm}}
    ...
    Proprietary Models (4/26, 15.4\%) &
    GPT-3.5/4, ChatGPT
    \cite{xu2024beyond,steinebach2024natural,wu2024generative,hao2025robust} \\
    Custom or Task-Specific Architectures (4/26, 15.4\%) &
    From-scratch or task-specific models
    \cite{yang2020vae,ding2023joint,ji2024principled,munyer2024deeptextmark} \\
    \hline
  \end{tabular}
\end{table}
