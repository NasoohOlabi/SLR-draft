% Improve page breaking and spacing for tables
\raggedbottom

\begin{table*}[!htbp]
\centering
\caption{Summary of Results from Reviewed Papers}
\Description{A comprehensive table summarizing the quantitative results and performance metrics from all reviewed papers on LLM-based steganography. The table includes metrics such as perplexity (PPL), bits per word (BPW), accuracy scores, and various steganographic evaluation measures for each paper, providing a comparative overview of the effectiveness of different approaches.}
...
\clearpage % Force page break between large tables

\begin{table*}[!htbp]
\centering
\caption{Models and Datasets Used in Reviewed Papers}
\Description{A detailed table listing the Large Language Models and datasets employed in each reviewed paper. The table shows the variety of models used (including BERT, GPT-2, LLaMA, and others) alongside their corresponding training and evaluation datasets, illustrating the diversity of experimental setups in LLM-based steganography research.}
...
\Description{A table analyzing the context-awareness capabilities of different steganographic approaches. The table categorizes papers based on whether they explicitly handle context, the type of contextual information they use (such as social media context or pre-text), and how they represent contextual information, providing insights into the evolution toward context-aware steganographic systems.}
