...
number  & result \\
\hline
1 & PPL: 28.879, $\textbackslash\{\}Delta$MP: 0.242, KLD: 3.302, JSD: 10.411, Acc: 0.600, R: 0.616 \\
\hline
...
\hline
8 & PPL: 8.81. JSDfull: 17.90 (x10\textbackslash\{\}textasciicircum\{\}-2). JSDhalf: 16.86 (x10\textbackslash\{\}textasciicircum\{\}-2). JSDzero: 13.40 (x10\textbackslash\{\}textasciicircum\{\}-2) TS-BiRNN: 80.29\%. R-BiLSTM-C: 84.34\%. BERT-C: 89.61\% \\
\hline
9 & Total Error: 0\%, Ave KLD: 0, Max KLD: 0, Ave PPL: 3.19 (EN), 7.49 (ZH), Capacity: 1.03–3.05 bits/token, Utilization: 0.66–0.74, Ave Time: \textbackslash\{\}textasciitilde\{\}4$\textbackslash\{\}mu$s/bit \\
\hline
10 & Bit acc: 0.994 (K=None), 1.000 (DAE), 0.978 (Adaptive+K=S); Meteor Drop: \textbackslash\{\}textasciitilde\{\}0.057; SBERT ↑: \textbackslash\{\}textasciitilde\{\}1.227; Ownership Rate: 1.0 (no attack), 0.978 (adaptive+K=S) \\
\hline
11 & BLEU: 30.5, PPL: 22.5, ER: 0.29, KL: 0.02, SIM: 0.86, Stego detection \textbackslash\{\}textasciitilde\{\}16\% \\
\hline
12 & 100\% accuracy (multi-synonym, 10-sentence), mSMS: 0.9892, TPR: 0.83, FNR: 0.17, Detection: 0.00188s, Insertion: 0.27931s \\
\hline
13 & ppl: 109.60, MAUVE: 0.2051, ER2: 10.42, $\textbackslash\{\}Delta$(cosine): 0.0088, $\textbackslash\{\}Delta$(simcse): 0.0191 \\
\hline
14 & Classifier Accuracy: 0.9880; Loop Count: 1.0160; PPL: 13.9565; Anti-Steganalysis Accuracy: \textbackslash\{\}textasciitilde\{\}0.5 \\
\hline
15 & [Not specified] \\
\hline
\cite{qiang2023natural} & LS07 P@1: 58.3, GAP: 65.1; CoInCo P@1: 62.6, GAP: 60.7; Text Recoverability: \textbackslash\{\}textasciitilde\{\}88–90\% \\
\hline
...
\hline
11 & BERT (encoder), LSTM (decoder) & WMT18 News Commentary (train/test), Yang et al. bits, Doc2Vec, 5,000 stego pairs (8:1:1 split) \\
\hline
...
\hline
\cite{qiang2023natural} & Transformer (Paraphraser), BART (BARTScore), BERT (BLEURT, comparisons) & ParaBank2, LS07, CoInCo, Novels, WikiText-2, IMDB, NgNews \\
\hline
...
\hline
number  & context aware & categ context & representation context & context usage in method detail text \\
\hline
...
\hline
10 & Yes; semantic-level embedding; synonym substitution using BERT & Yes; watermark message assigned categorical label (e.g., 4-bit → 1-of-16) & Yes; semantic embeddings via transformer encoder and BERT; SBERT distance as metric & [Not specified] \\
\hline
...
\hline
14 & implicit & Text & Semanteme ($\textbackslash\{\}alpha$) as a vector in semantic spac & The semanteme ($\textbackslash\{\}alpha$) shapes the generated text to match a topic, ensuring it appears natural and blends into regular communication. This contextual alignment helps conceal the message and enables the classifier to recover it using the same semantic grounding. \\
\hline
...
\hline
number  & Type & Main strengths & Main weaknesses \\
\hline
1 & Steganography & statistical fidelity & Not AutoRegressive \\
\hline
2 & Steganography & [Not specified] & [Not specified] \\
\hline
3 & Steganography & High capacity, fluency, semantic relevance, EES boosts entropy, $\textbackslash\{\}epsilon$-security satisfied & Context limits capacity, retrieval vulnerable to mismatch, small models limit query handling \\
\hline
...
\hline
\cite{qiang2023natural} & Watermarking & High meaning preservation, strong performance vs. BERT-based NLW, no annotated data needed, robust decoding, high text recoverability & Limited reversibility, hard for short texts, beam decoding limits substitution, lower capacity in some cases \\
\hline
...
\hline
number  & pipline method used \\
\hline
1 & Input text → Encoder (LSTM/BERT) → latent vector z \textbackslash\{\}textasciitilde\{\} N(0,1) z → Decoder (LSTM) → conditional distribution → top-m Candidate Pool Candidate Pool + secret bits → Coding (Huffman/Arithmetic) → word output Repeat → sentence with hidden bits Shared z + Decoder → conditional distribution → CP reconstruction Observed word + CP → decode bits \\
\hline
...
\hline
4 & pre‑text → LSTM → Temporal context (H\_time) → self-attention → H\_time'                                   Temporal context (H\_time) + sliding-window graph                         → GAT → Spatial context (H\_space)  GAT + H\_time' + H\_space → fusion → next-token distribution \\
\hline
...
\hline
7 & Next-word distribution → partitioned into intervals → random r selects word → shared r prefix → message bits → r XOR PRG mask → secure transmission → Receiver unmasks r → decodes bits → Bit rate $\textbackslash\{\}propto$ entropy of next word \\
\hline
8 & Secret text → Huffman + Edge Flipping → binary bitstream   → LM generates candidates (filtered by threshold $\textbackslash\{\}tau$)   → Embedding Module selects word matching bitstream prefix   + Annealing Selection (temperature control)   + Repeat Penalty (discourages repetition)   → Word output per step → encoded sentence \\
\hline
...
\hline
10 & Original Text (S), Watermark bits (m), Side info (K) → Encoder (Transformer) → paraphrase under distortion constraints → Watermarked Text (X) Attacker (omniscient) → corrupts X → Corrupted Text (Y) Decoder (Transformer + linear) + K → estimate watermark (m\textbackslash\{\}textasciicircum\{\}) Train via rate-distortion game maximizing mutual info, minimizing distortion, with adversarial robustness \\
\hline
...
\hline
13 & Secret message m → split into data info + control info Data info → retrieve carrier c from corpus C → maximize m $\textbackslash\{\}cap$ c Control info = (keywords k = m \textbackslash\{\} c, positions i = m $\textbackslash\{\}cap$ c) → format → bitstream b Bitstream b + keywords k + carrier c → Prompt Learning + Keyword Guidance → generate stegotext s s embeds keywords k + hidden bitstream b → high semantic coherence ($\textbackslash\{\}Delta$cos, $\textbackslash\{\}Delta$simcse) → cognitive imperceptibility Only c or s $\textbackslash\{\}neq$ full m → layer separation → security ↑, anti-steganalysis ↑ Embedding rate (ER2) ↑ → high payload per word with low perplexity \\
\hline
14 & Secret message → Semantic mapping → semanteme $\textbackslash\{\}alpha$ (discrete space, log(n)-bit) $\textbackslash\{\}alpha$ → CTRL (Transformer) → stegotext x (conditional distribution p$\textbackslash\{\}theta$(x|$\textbackslash\{\}alpha$)) Stegotext x → BERT classifier → extracted semanteme $\textbackslash\{\}alpha$' $\textbackslash\{\}alpha$' $\textbackslash\{\}neq$ $\textbackslash\{\}alpha$ → Rejection sampling → repeat generation until $\textbackslash\{\}alpha$' = $\textbackslash\{\}alpha$ Final stegotext → Bob decodes → secret message m' \\
\hline
15 & Key Generator: Topic-specific word sets → labeled subsets (e.g., A, B) → no duplicates, single words only Message → sequence of subset labels (stegosequence, e.g., "AABBAABB") Embedder (ChatGPT 4.0): Stegosequence + topic + word sets → stego cover text \\
\hline
...
\hline
17 & Cover text (Y) + Conditional Code → BART Encoder → encoded representation Secret message (S) → bit stream → 2\textbackslash\{\}textasciicircum\{\}n vocabulary groups, each with n-bit code BART Decoder + Group-Wise Masked Decoding → mask non-aligned token probabilities → stego text (Y') Beam search → K candidate stego texts GPT2 → Perplexity (PPL) scoring → select lowest PPL candidate as final stego text \\
\hline
\end{tabular}
}
\end{table*}
